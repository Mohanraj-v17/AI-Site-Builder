...
export const makeRevision = async (req, res) => {
    ...
    try {
        ...
        const enhancedPrompt = promptEnhanceResponse.choices[0].message.content?.trim() || "";
        await prisma.conversation.create({
            data: {
                role: 'assistant',
...
`
                ...
                {
                    role: 'user',
                    content: `Here is the enhanced prompt: "${currentProject.current_code}" The user wants this change: "${enhancedPrompt}"`
                }
            ]
        });
...
export const saveProjectCode = async (req, res) => {
    try {
        ...
        res.json({ code: "Project saved successfully" });
    }
    catch (error) {
        console.log(error.code || error.message);
        res.status(500).json({ message: error.message });
