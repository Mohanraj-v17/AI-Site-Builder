           ...
           <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
                    ...
                    if(isMessage){
                        ...
                        return (
                            <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                                ...
                                <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-realxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100" }`}>
                                    {msg.content}
                                </div>
                                {isUser && (
                                    <div className='w-8 h-8 rounded-full ng-gray-700 flex items-center justify-center'>
            ...
            <form onSubmit={handleRevisions} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                 ...
                 <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 teext-white transition-colors disabled:opacity-60'>
                    {isGenerating ?
